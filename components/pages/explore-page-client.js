...
export default function ExplorePageClient({
  ...
  /* ── shared filter logic ── */
  function clearFilters() { setQuery(""); setActiveProvince("All"); }
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 32 }}>
            <div style={{ flex: 1 }}>
              ...
              <h1 style={{ fontSize: 38, fontWeight: 900, color: "#fff", lineHeight: 1.05, letterSpacing: "-0.025em", marginBottom: 8 }}>
                Explore All <span style={{ color: "#86efac" }}>77 Districts</span>
              </h1>
              <p style={{ fontSize: 15, color: "rgba(255,255,255,0.72)", lineHeight: 1.6, maxWidth: 440 }}>
                Seven provinces. 77 districts. Endless stories — from Himalayan summits to Terai plains.
              </p>
            </div>
            {/* Stats */}
            <div style={{ display: "flex", gap: 14, flexShrink: 0 }}>
              {[
                ...
                <div key={label} style={{ background: "rgba(255,255,255,0.12)", backdropFilter: "blur(8px)", border: "1px solid rgba(255,255,255,0.2)", borderRadius: 16, padding: "14px 20px", textAlign: "center", minWidth: 80 }}>
          ...
          <div style={{ width: 220, flexShrink: 0, position: "sticky", top: 24 }}>
            ...
            <div style={{ background: "#fff", borderRadius: 16, border: "1.5px solid #e2e8f0", overflow: "hidden", boxShadow: "0 2px 10px rgba(15,23,42,0.04)" }}>
              ...
              {allProvinces.map((province, i) => {
                ...
                const count = province === "All" ? totalDistricts : (districtsByProvince?.[province]?.length || 0);
                return (
                  <button
                    key={province}
                    type="button"
                    ...
                    style={{
                      ...
                      borderBottom: i < allProvinces.length - 1 ? "1px solid #f8fafc" : "none",
                      background: active ? "linear-gradient(90deg, rgba(5,150,105,0.08), rgba(5,150,105,0.03))" : "transparent",
            ...
            {(query || activeProvince !== "All") && (
              <button type="button" onClick={clearFilters}
                style={{ width: "100%", marginTop: 12, padding: "10px", borderRadius: 12, border: "none", background: "#ecfdf5", color: "#059669", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
                Clear filters
              </button>
            )}
            ...
            {search || activeProvince !== "All" ? (
              ...
              <div>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
                  <p style={{ fontSize: 14, color: "#64748b" }}>
                    ...
                    {search
                      ? ` result${filteredDistricts.length !== 1 ? "s" : ""} for "${deferredQuery.trim()}"`
                      : ` district${filteredDistricts.length !== 1 ? "s" : ""} in ${activeProvince}`}
                  </p>
                </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 44 }}>
                {provinces.map((province) => {
                  ...
                  const visible = isExpanded ? provinceDistricts : provinceDistricts.slice(0, 6);
                  const canExpand = provinceDistricts.length > 6;
                  const emoji = PROVINCE_EMOJIS[province] || "📍";
                    ...
                    <section key={province}>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                          <div style={{ width: 44, height: 44, borderRadius: 14, background: "linear-gradient(135deg, #ecfdf5, #d1fae5)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, flexShrink: 0 }}>
                            {emoji}
                          </div>
                          <div>
                            <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.18em", textTransform: "uppercase", color: "#059669", marginBottom: 2 }}>Province</p>
                            <h2 style={{ fontSize: 22, fontWeight: 800, color: "#0f172a", lineHeight: 1.1, letterSpacing: "-0.015em" }}>{province}</h2>
                          </div>
                        </div>
                        <button type="button" onClick={() => setActiveProvince(province)}
                          style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 13, fontWeight: 700, color: "#059669", background: "#ecfdf5", borderRadius: 999, padding: "7px 16px", border: "none", cursor: "pointer" }}>
                          All {provinceDistricts.length} <ChevronRightIcon className="size-4" />
                        </button>
                      </div>
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 18 }}>
                        {visible.map((d) => <DistrictCard key={d.id} district={d} />)}
                      </div>
                      {canExpand && (
                        <div style={{ marginTop: 14, display: "flex", justifyContent: "center" }}>
                          <button type="button"
                            onClick={() => setExpandedProvinces((p) => ({ ...p, [province]: !p[province] }))}
                            style={{ border: "1.5px solid #e2e8f0", background: "#fff", color: "#059669", borderRadius: 999, padding: "10px 24px", fontSize: 13, fontWeight: 700, cursor: "pointer", boxShadow: "0 2px 8px rgba(15,23,42,0.05)" }}>
      ...
      <div className="lg:hidden">
        ...
        <div className="fade-up-1 scrollbar-hide" style={{ display: "flex", gap: 8, padding: "14px 20px 0", overflowX: "auto" }}>
          {allProvinces.map((province) => {
            ...
            const emoji = province === "All" ? "🗺️" : (PROVINCE_EMOJIS[province] || "📍");
            return (
              <button key={province} type="button" onClick={() => setActiveProvince(province)}
          ...
          {search || activeProvince !== "All" ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 32 }} className="fade-up-2">
              {provinces.map((province) => {
                ...
                if (!provinceDistricts.length) return null;
                const isExpanded = Boolean(expandedProvinces[province]);
                const visibleDistricts = isExpanded ? provinceDistricts : provinceDistricts.slice(0, INITIAL_DISTRICT_COUNT);
                  ...
                  <section key={province}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        <div style={{ width: 40, height: 40, borderRadius: 12, background: "linear-gradient(135deg, #ecfdf5, #d1fae5)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, flexShrink: 0 }}>{emoji}</div>
                        <div>
                    ...
                    {canExpand && (
                      <div style={{ marginTop: 12, display: "flex", justifyContent: "center" }}>
                        <button type="button"
                          ...
                          style={{ border: "1.5px solid #e2e8f0", background: "#fff", color: "#059669", borderRadius: 999, padding: "9px 20px", fontSize: 12, fontWeight: 700, cursor: "pointer", boxShadow: "0 2px 8px rgba(15,23,42,0.05)" }}>
                          {isExpanded ? "Show less" : `Show all ${provinceDistricts.length} districts`}
                        </button>
                      </div>
                    )}
