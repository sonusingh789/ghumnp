...
function normalizeImageUrl(url, { width = 600, quality = 75 } = {}) {
  ...
  // ImageKit: append transformation params (w, q, format=auto) if not already present
  if (url.includes("ik.imagekit.io") && !url.includes("tr=")) {
    const sep = url.includes("?") ? "&" : "?";
    return `${url}${sep}tr=w-${width},q-${quality},f-auto`;
  }

  // Unsplash: replace or add w= and q= params for smaller payload
...
function mapDistrict(row) {
  return {
    ...
    image: normalizeImageUrl(row.image_url) || "https://images.unsplash.com/photo-1544735716-392fe2489ffa?w=600&auto=format&fit=crop",
...
function mapPlace(row) {
  return {
    ...
    image: normalizeImageUrl(row.cover_image_url) || "https://images.unsplash.com/photo-1544735716-392fe2489ffa?w=600&auto=format&fit=crop",
    images: row.images?.length
      ? row.images.map((u) => normalizeImageUrl(u, { width: 1200, quality: 80 }))
      : row.cover_image_url
        ? [normalizeImageUrl(row.cover_image_url, { width: 1200, quality: 80 })]
        : [],
    rating: Number(row.rating || 0),
