...
const getCachedRecentPlaces = unstable_cache(
  async () => {
    const result = await query(
      `SELECT TOP 100 pl.slug, pl.name, pl.category, pl.cover_image_url, pl.rating, pl.description,
              pl.location_text, pl.is_featured, pl.is_hidden_gem, d.slug AS district_slug
       FROM Places pl
       INNER JOIN Districts d ON d.id = pl.district_id
       WHERE pl.status = 'approved'
...
const getCachedHomePageCollections = unstable_cache(
  async () => {
    const topPlacesResult = await query(
      `SELECT TOP 20 pl.slug, pl.name, pl.category, pl.cover_image_url, pl.rating, pl.description,
       ...
       ORDER BY pl.rating DESC, pl.review_count DESC, pl.name`
    );

    return {
      topPlaces: topPlacesResult.recordset.map(mapPlaceSummary),
    };
  },
...
export async function getDistrictListingPlacesByDistrictSlug(districtSlug) {
  try {
