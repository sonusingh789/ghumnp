...
const getCachedHomePageCollections = unstable_cache(
  async () => {
    // Run both queries in parallel — previously sequential, wasting ~200ms per request
    const [popularDistrictsResult, topRatedPlacesResult] = await Promise.all([
      query(
    ...
    return {
      ...
      topPlaces: topRatedPlacesResult.recordset.map(mapPlaceSummary),
    };
  },
  ...
  async (placeSlug) => {
    const result = await query(
      `SELECT TOP 1 pl.id, pl.slug, pl.name, pl.category, pl.cover_image_url, pl.rating, pl.description,
              pl.location_text, pl.is_featured, pl.is_hidden_gem, pl.is_verified,
              pl.updated_at, pl.created_at, d.slug AS district_slug,
...
/** Minimal district data for the hero carousel — 5 random slides per request.
 *  Shuffled server-side so SSR and hydration see the same order (no mismatch). */
export async function getDistrictCarouselSlides() {
  try {
    const districts = await getCachedDistrictCards();
