  ...
  try {
    ...
    await query(
      `INSERT INTO Users (name, email, password_hash, created_at)
       VALUES (@name, @email, @password_hash, SYSDATETIME())`,
    ...
    if (err.message?.includes("column 'role'") || err.message?.includes('column "role"')) {
      return NextResponse.json(
        { error: 'Database schema is outdated. Remove the role column requirement from the Users table.' },
        { status: 500 }
      );
    }
